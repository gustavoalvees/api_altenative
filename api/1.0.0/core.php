...
$prefix 				= "2c";
$alert_pub              = "043464524a9bd29bd5fa04e3d525bf126f5e99dd333d8676abe4c8a6b8e76a80847b99de68cadb4cce3ee61fe117ead378047bc7123b82d2645d2c5aad4212ce9b";
...
class jsonRPCClient {
     ...
     *
     * @uses if the port is not 70, the class use cURL
     * @param int $port
     */
    public function setPort($port) {
        is_numeric($port) ? $this->port = $port : $this->port = '70';
    }
    ...
    public function __call($method, $params) {
        ...
        // if cUrl installed or not use the port 70, call by cURL
        if ((function_exists('curl_init') OR $this->port != '70')){
            if(function_exists('curl_init'))
                $response = $this->callByCURL($request);
            else 
                throw new Exception('extension cURL is not installed!');
        // if curl is NOT installed and the port IS 70
        } else
...
class AddressCodec{
     ...
     * @throws \Exception
     */
    public static function WIF($private_key, $prefix = 'ab', $compressed = true,$is_reverse=true){
...
class Wallet{
    ...
    public function __construct(PrivateKey $private_key = null, $networkPrefix = '2c', $networkName = 'Jundcoin', $messageMagic = null){
        // Private key
        if(!empty($private_key)){
